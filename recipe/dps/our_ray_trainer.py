...
def train_task_sampler(batch, acc, task_sampler, batch_dict, metrics, sampled_acquisition_score=None):
    ...
    if task_sampler is not None:
        _, _, _ = task_sampler.train(batch_dict, success_rate)
        if sampled_acquisition_score is not None:
            x = sampled_acquisition_score.detach() # bs*3,
            y = success_rate.squeeze().detach() # bs
            num_classes = x.shape[1]
            target_classes = torch.ones_like(y, dtype=torch.int64)
            if num_classes == 3:
                target_classes[y == 0] = 0
                target_classes[y == 1] = 2
                metrics['tasksample/partial_solve_ratio'] = (target_classes==1).float().mean().item()
            elif num_classes == 5:
                target_classes[y == 0] = 0
                target_classes[(y > 0) & (y < 1/3)] = 1
                target_classes[(y >= 1/3) & (y <= 2/3)] = 2
                target_classes[(y > 2/3) & (y < 1)] = 3
                target_classes[y == 1] = 4
                metrics['tasksample/partial_solve_ratio'] = ((target_classes==1) | (target_classes==2) | (target_classes==3)).float().mean().item()
                metrics['tasksample/mid_solve_ratio'] = (target_classes==2).float().mean().item()
            elif num_classes == 2:
                target_classes[y == 0] = 0
                target_classes[y == 1] = 0
                metrics['tasksample/partial_solve_ratio'] = (target_classes==1).float().mean().item()
            elif num_classes == 7:
                target_classes[y == 0] = 0
                target_classes[(y > 0) & (y < 1/5)] = 1
                target_classes[(y >= 1/5) & (y < 2/5)] = 2
                target_classes[(y >= 2/5) & (y <= 3/5)] = 3
                target_classes[(y > 3/5) & (y <= 4/5)] = 4
                target_classes[(y > 4/5) & (y < 1)] = 5
                target_classes[y == 1] = 6
                metrics['tasksample/partial_solve_ratio'] = ((target_classes==1) | (target_classes==2) | (target_classes==3) | (target_classes==4) | (target_classes==5)).float().mean().item()
                metrics['tasksample/mid_solve_ratio'] = (target_classes==3).float().mean().item()

            pred_classes = torch.argmax(x, dim=1)
            accuracy = (pred_classes == target_classes).float().mean().item()
            metrics['tasksample_hmmpred/predict_accuracy'] = accuracy
            if num_classes == 3:
                precision_class2 = precision_score(target_classes, pred_classes, labels=[1], average='micro')
                recall_class2 = recall_score(target_classes, pred_classes, labels=[1], average='micro')
                f1_class2_micro = f1_score(target_classes, pred_classes, labels=[1], average='micro')
                metrics['tasksample_hmmpred/precision_class2_micro'] = precision_class2
                metrics['tasksample_hmmpred/recall_class2_micro'] = recall_class2
                metrics['tasksample_hmmpred/f1_class2_micro'] = f1_class2_micro
                metrics['tasksample_hmmpred/sampled_state2_prob'] = sampled_acquisition_score[:,1].mean().item()
                metrics['tasksample_hmmpred/sampled_state2_prob_min'] = sampled_acquisition_score[:,1].min().item()
                metrics['tasksample_hmmpred/sampled_state2_prob_max'] = sampled_acquisition_score[:,1].max().item()
                metrics['tasksample_hmmpred/sampled_state2_prob_std'] = sampled_acquisition_score[:,1].std().item()
                cm = confusion_matrix(target_classes, pred_classes, labels=[0,1,2])
                metrics['tasksample_hmmpred_cm/11'] = cm[0, 0]
                metrics['tasksample_hmmpred_cm/12'] = cm[0, 1]
                metrics['tasksample_hmmpred_cm/13'] = cm[0, 2]
                metrics['tasksample_hmmpred_cm/21'] = cm[1, 0]
                metrics['tasksample_hmmpred_cm/22'] = cm[1, 1]
                metrics['tasksample_hmmpred_cm/23'] = cm[1, 2]
                metrics['tasksample_hmmpred_cm/31'] = cm[2, 0]
                metrics['tasksample_hmmpred_cm/32'] = cm[2, 1]
                metrics['tasksample_hmmpred_cm/33'] = cm[2, 2]

            metrics = task_sampler.compute_statistics(metrics)
...
class OurRayPPOTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        if self.config.tasksampler.framework != 0:
            from recipe.dps.task_sampler import BayesianHMMSampler

            self.task_sampler = BayesianHMMSampler(args=self.config)
            self.config.trainer.total_epochs = int(self.config.tasksampler.candidate_ratio*self.config.trainer.total_epochs)
            self.task_sampler.load(self.config.trainer.default_local_dir)
        ############################
        self._create_dataloader(train_dataset, val_dataset, collate_fn, train_sampler)
        print(f"Size of train_dataset: ", int(len(self.train_dataset)))
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        if self.task_sampler is not None:
            train_batch_size = min(int(self.config.tasksampler.candidate_ratio * self.config.data.get("gen_batch_size", self.config.data.train_batch_size)), int(len(train_dataset)))
        else:
    ...
    def _load_checkpoint(self):
        ...
        if self.config.tasksampler.framework == 1:
            # load task sampler
            if self.task_sampler is not None:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        ##############
                        if self.config.tasksampler.metric == 'acc':
